...
/-! ### The box closure: both defect measures are defects of ONE operator

The two invariants compared above — the Čech obstruction and the
rectangle-cover complexity `ρ` — are not unrelated numbers that happen to
disagree.  They are the two standard defects of a *single* closure
operator on `Set (∀ v, A v)`, the **box closure**

    `boxClosure T = ∏ᵥ (projᵥ T)`,

the smallest rectangle containing `T`.  It is literally
`compatibleFamilies T` of `Ste.CechObstruction`
(`boxClosure_eq_compatibleFamilies`, `rfl`), and it is a closure operator
in the textbook sense: extensive (`subset_boxClosure`), monotone
(`boxClosure_mono`), idempotent (`boxClosure_idem`), with fixed points
exactly the rectangles (`boxClosure_eq_self_iff_rectangular`).

Given any closure operator there are two canonical ways to measure how
far a set is from being closed:

* the **additive defect** `|cl T \ T|` — how many points the closure adds.
  Here this is exactly the Čech obstruction
  (`cechObstruction_eq_boxClosure_sub_encard`, a restatement of
  `cechObstruction_eq_box_sub_encard`).
* the **covering defect** — the least number of CLOSED sets needed to
  cover `T` exactly.  Here this is exactly `ρ(T)`, because the
  boxClosure-fixed sets are precisely the rectangles
  (`hasRectCover_iff_boxClosure_fixed`, `rectCoverNumber_eq_boxClosureFixedCoverNumber`).

Both defects vanish on the same sets — the closed ones — which is why
`rectCoverNumber_eq_one_iff_cechVanishes` pins them together at the
bottom of the scale.  But an additive defect and a covering defect are
different *kinds* of measurement (one counts points, the other counts
pieces, and the number of pieces is roughly multiplicative where the
number of points is additive), so they are free to diverge in magnitude
above that bottom.  This is precisely why the unification here does NOT
contradict the incomparability results
`not_forall_rectCoverNumber_le_cechObstruction` and
`not_forall_cechObstruction_le_rectCoverNumber`: sharing a closure
operator forces agreement on the *fixed points*, never on the *size* of
the two defects.

**Remark (Formal Concept Analysis).**  The box closure is a concept
closure in the sense of `Ste.HyperFrame`, which reads STE feasibility as
Formal Concept Analysis (Ganter–Wille; `Order.Concept` in Mathlib).  Take
the polarity between points `f : ∀ v, A v` and coordinate-value pairs
`(v, a)`, with incidence `f ⊨ (v, a) ↔ f v = a`.  Reading the attribute
side as the *complements* of the coordinate-value pairs (equivalently,
generating extents from the per-coordinate value sets realized in `T`),
the extent generated by a set `T` of points is exactly
`∏ᵥ projᵥ T = boxClosure T`: a point is in the closure iff every one of
its coordinate values is witnessed somewhere in `T`.  So the rectangles
are the extents of the concept lattice of this polarity, and both defect
measures above are defects of the FCA closure.  (Remark only — no
`Concept` machinery is introduced here.) -/

/-- The **box closure** of `T`: the product of its per-coordinate
projections, i.e. the smallest rectangle containing `T`.  Definitionally
equal to `compatibleFamilies T` (`Ste.CechObstruction`); named separately
here to make its role as a closure operator explicit. -/
def boxClosure (T : Set (∀ v, A v)) : Set (∀ v, A v) :=
  Set.univ.pi fun v => (fun f => f v) '' T

/-- The box closure IS the set of compatible families of the singleton
cover — the two notions coincide by definition. -/
theorem boxClosure_eq_compatibleFamilies (T : Set (∀ v, A v)) :
    boxClosure T = compatibleFamilies T := rfl

/-- The box closure of `T` is a rectangle — by construction. -/
theorem isRectangle_boxClosure (T : Set (∀ v, A v)) :
    IsRectangle (boxClosure T) :=
  ⟨fun v => (fun f => f v) '' T, rfl⟩

/-- **Closure operator, extensivity**: `T ⊆ boxClosure T`. -/
theorem subset_boxClosure (T : Set (∀ v, A v)) : T ⊆ boxClosure T :=
  subset_compatibleFamilies T

/-- **Closure operator, monotonicity**. -/
theorem boxClosure_mono {S T : Set (∀ v, A v)} (h : S ⊆ T) :
    boxClosure S ⊆ boxClosure T :=
  Set.pi_mono fun _ _ => Set.image_mono h

/-- **Closure operator, fixed points**: the boxClosure-fixed sets are
exactly the rectangles.  This is the hinge of the unification: the
"closed" sets of the box closure are the product sets. -/
theorem boxClosure_eq_self_iff_rectangular {T : Set (∀ v, A v)} :
    boxClosure T = T ↔ IsRectangle T := by
  constructor
  · intro h
    exact h ▸ isRectangle_boxClosure T
  · rintro ⟨P, rfl⟩
    exact compatibleFamilies_pi P

/-- **Closure operator, idempotence**: `boxClosure` is a closure operator
on `Set (∀ v, A v)`. -/
theorem boxClosure_idem (T : Set (∀ v, A v)) :
    boxClosure (boxClosure T) = boxClosure T :=
  boxClosure_eq_self_iff_rectangular.mpr (isRectangle_boxClosure T)

/-- **Minimality**: `boxClosure T` is the SMALLEST rectangle containing
`T`, justifying the name "closure". -/
theorem boxClosure_subset_of_rectangle {T R : Set (∀ v, A v)}
    (hR : IsRectangle R) (h : T ⊆ R) : boxClosure T ⊆ R :=
  (boxClosure_mono h).trans_eq (boxClosure_eq_self_iff_rectangular.mpr hR)

/-- **Defect 1 (additive)**: the Čech obstruction is the additive defect
`|cl T| − |T|` of the box closure.  Restatement of
`cechObstruction_eq_box_sub_encard` through `boxClosure`. -/
theorem cechObstruction_eq_boxClosure_sub_encard (T : Set (∀ v, A v)) :
    cechObstruction T = (boxClosure T).encard - T.encard :=
  cechObstruction_eq_box_sub_encard T

/-- **Defect 2 (covering), the restatement**: an exact rectangle cover of
`T` is exactly an exact cover of `T` by boxClosure-FIXED sets.  This is
`HasRectCover` read through `boxClosure_eq_self_iff_rectangular`; no new
covering machinery is introduced. -/
theorem hasRectCover_iff_boxClosure_fixed {T : Set (∀ v, A v)} {m : ℕ} :
    HasRectCover T m ↔
      ∃ R : Fin m → Set (∀ v, A v),
        (∀ i, boxClosure (R i) = R i) ∧ ⋃ i, R i = T :=
  ⟨fun ⟨R, hrect, hunion⟩ =>
    ⟨R, fun i => boxClosure_eq_self_iff_rectangular.mpr (hrect i), hunion⟩,
   fun ⟨R, hfix, hunion⟩ =>
    ⟨R, fun i => boxClosure_eq_self_iff_rectangular.mp (hfix i), hunion⟩⟩

/-- **Defect 2 (covering), as an infimum**: `ρ(T)` is the least number of
boxClosure-closed sets covering `T` exactly. -/
theorem rectCoverNumber_eq_boxClosureFixedCoverNumber
    (T : Set (∀ v, A v)) :
    rectCoverNumber T =
      sInf ((fun m : ℕ => (m : ℕ∞)) ''
        {m : ℕ | ∃ R : Fin m → Set (∀ v, A v),
          (∀ i, boxClosure (R i) = R i) ∧ ⋃ i, R i = T}) := by
  unfold rectCoverNumber
  congr 1
  congr 1
  ext m
  exact hasRectCover_iff_boxClosure_fixed

/-- The cardinality of the box closure factorizes over coordinates:
`|cl T| = ∏ᵥ |projᵥ T|` (finitely many variables).  With
`cechObstruction_eq_boxClosure_sub_encard` this computes the additive
defect from the projection sizes alone. -/
theorem boxClosure_encard_eq [Fintype V] (T : Set (∀ v, A v)) :
    (boxClosure T).encard = ∏ v, ((fun f => f v) '' T).encard :=
