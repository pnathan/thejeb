...
\texttt{Ste.Convex}. The fuzzy/graded case (Eq.~(3) with
$\Psi_i:\Xi\to[0,1]$) is machine-checked in \texttt{Ste.FuzzyFrame}
(\texttt{consistencyDegree}, \texttt{cut\_fuzzyFeasibility},
\texttt{weightedFrame\_gradedHelly\_two}); the projection algorithms
remain future work.
...
is machine-checked in \texttt{Ste.Carlson.Cipher}; the residual-key
counting of Lemma~4.1 is done
(\texttt{card\_consistent\_keys}, \texttt{Ste/Carlson/Counting.lean}),
and the reduction chain Theorem~5.1/5.2/Corollary~5.3 is done
(\texttt{Ste/Carlson/Reduction.lean}), as is deterministic asymptotic
unicity (\texttt{asymptoticUnicity\_iff\_unicity},
\texttt{Ste/Carlson/Asymptotic.lean}). Remaining: Theorem~4.1 (the
$P$-cipher key count) and the AEP/entropy layer (Theorem~2.3, and the
almost-sure form of asymptotic unicity, which needs a probability space
on message streams).
