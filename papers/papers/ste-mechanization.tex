...
revision --- Carlson's key-counting Lemma~4.1 and the cipher-reduction
chain (Theorem~5.2, Lemma~5.1, Corollary~5.3). Every theorem
...
\[ \#\{\tau\in\mathrm{Perm}\,A : \forall a\in T,\ \tau a=\sigma a\}
   = (|A|-|T|)!, \]
the size of the STE feasible key set after one observation.
\end{theorem}
...
\begin{theorem}[\texttt{STE.Carlson.permutation\_reduces\_to\_substitution},
\texttt{STE.Carlson.coperm\_injective}]
Every permutation-cipher key is a substitution-cipher key, and the
realization $\mathrm{coperm}$ is \emph{injective}: distinct position
permutations induce distinct value permutations. This upgrades
Theorem~5.2 to a genuine embedding of permutation ciphers into
...
\end{theorem}
\begin{theorem}[\texttt{STE.Carlson.psp\_reduces\_to\_substitution}]
A permutation--substitution--permutation composition is again a single
value permutation: every block cipher is a block substitution cipher
(Corollary~5.3).
\end{theorem}
...
\section{Future work}
With Lemma~4.1 and the reduction chain (Theorems~5.2, Lemma~5.1,
Corollary~5.3) now discharged, the remaining targets, in increasing
depth, are: Carlson's general counting Theorem~4.1 (the
