...
\begin{theorem}[Meet is not --- and has no uniform Lipschitz constant]
There exist partitions $P,Q,R$ of a four-element carrier with
$d(P,Q) < d(P\sqcap R,\; Q\sqcap R)$
(\code{shannonDist\_inf\_not\_contraction}).  More: for \emph{every}
constant $C$ there exist $P,Q,R$ --- on the $2k$-element carrier with
$k=C+1$, taking $P=\top$, $Q$ the two halves, $R$ the perfect matching
--- with
$C\cdot d(P,Q) < d(P\sqcap R,\; Q\sqcap R)$, because
$d(P,Q)=1$ throughout the family while $d(P\sqcap R,\;Q\sqcap R)=k$ grows
without bound (\code{shannonDist\_meetWitness\_eq}).  The meet operation
therefore admits \emph{no} Lipschitz constant.  \emph{(Both mechanized:
\code{shannonDist\_inf\_not\_contraction} and the general-$k$
\code{meet\_not\_lipschitz}, \Ste{MeetScaling}.)}
\end{theorem}
