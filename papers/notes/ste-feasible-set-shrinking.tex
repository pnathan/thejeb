...
product sets (\emph{rectangles}) whose union is the family --- is
sandwiched by machine checked bounds.  It is at most the family's
cardinality (\path{rectCoverNumber_le_encard}).  For the lower bound,
call a subset $F$ of the family a \emph{fooling set}
(\path{IsFoolingSet}) when no single rectangle contained in the family
holds two distinct points of $F$; an exact cover must then spend a
separate rectangle on each point of $F$
(\path{IsFoolingSet.encard_le_of_hasRectCover}), so the cover number is
at least the size of any fooling set
(\path{foolingNumber_le_rectCoverNumber}).  Finally it equals $1$
exactly when the \v{C}ech obstruction of the singleton cover vanishes
...
diagnostic: a fooling set of size $k$ exhibited in the shrunken family
certifies that no rectangular (per-variable factored) representation
with fewer than $k$ pieces exists.
