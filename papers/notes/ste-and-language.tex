...
two pieces of information is \emph{unification}.  Two dual orders are
in play, and it is worth keeping them apart.  In the \emph{subsumption}
order of \citet{carpenter1992logic}---where $F \sqsubseteq G$ means $F$
is less informative than (subsumes) $G$---unification is the
\emph{least upper bound} $F \sqcup G$, when it exists: it is the least
informative structure carrying all the information of both.  In the
dual \emph{denotational} order, each feature structure $F$ denotes the
set $\sem{F}$ of fully specified objects it subsumes, more information
means a smaller set, and unification is the \emph{meet}---ordinary
intersection:
\[
\sem{F \sqcup G} \;=\; \sem{F} \cap \sem{G} .
\]
...
unification playing the role of adding an item of information (a join
$\sqcup$ on structures, a meet $\cap$ on their denotations).
...
\begin{theorem}[signature fibers;
\lean{STE.indisc\_iff\_upperPolar\_singleton\_eq}]
$a$ and $b$ are indiscernible iff they have the same object intent:
$\mathrm{upperPolar}(\mathrm{sat}\,S)\,\{a\} =
 \mathrm{upperPolar}(\mathrm{sat}\,S)\,\{b\}$.  The Pawlak partition of
$\Xi$ is thus exactly the pullback partition of the FCA object-intent
map, tying Section~\ref{sec:hyperframe} and this section together.
\end{theorem}
...
We flag this explicitly as \emph{not yet proven}.  Sheaf-theoretic
machinery does exist in the library---the presheaf/sheaf condition and
gluing (\texttt{Ste.Sheaf}, \texttt{Ste.LocalGluing}), \v{C}ech covers
and their obstruction (\texttt{Ste.CechCover}, with
\texttt{rectangular\_cechVanishesCover}; \texttt{Ste.CechComplex},
\texttt{Ste.TwistedCech}).  What is \emph{not} formalized is precisely
what this conjecture asks for: a site over the \emph{constraint} poset
(with covers given by unions, or a Grothendieck topology on the
document poset), and a representation theorem recovering the
hyperframe lattice as its lattice of closed subsheaf sections, and
