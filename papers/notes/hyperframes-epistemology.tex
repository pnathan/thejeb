...
\emph{set-valued frame}, and argue it is the right primitive precisely
...
universal deep meaning in the Chomskyan lineage), and is honest about
...
correspondence'' and keeping ``coherence as criterion'' makes natural
and provable --- and, in the central cases, genuinely forces: lattice
theory and Formal Concept Analysis (whose formal concepts are the
\emph{hyperframes} of the title), rough-set indiscernibility, monotone
update semantics, a paraconsistent reading of local contradiction, a
genuine metric geometry on readings, and local-to-global criteria for
when coherence checking localizes.
...
\section{The primitive is the set-valued frame, not the crisp frame}
\label{sec:primitive}
...
\textbf{Not a universal deep meaning either.} A different,
non-Platonic universalism is also declined: the strongest reading of a
picture often built on Chomskyan deep structure
\citep{chomsky1965aspects} --- that every utterance resolves, in a
competent mind, to one shared underlying representation across
speakers, languages, and eras, of which surface texts are mere
transformations. (\emph{Aspects} itself posits deep structure as
language-internal syntax; the cross-linguistic meaning claim is its
inheritors' extension, and it is that extension being declined
here.) That picture chases a single semantics
...
provable content. Six consequences follow, each with a machine-checked
piece already in the Lean development. They are not all forced in the
same sense, and it is worth saying which is which: the first three ---
the concept lattice, the indiscernibility floor, monotone update ---
are automatic consequences of taking a set-valued primitive at all,
via the Galois connection every relation carries; the paraconsistent
reading and the partition-rank metric are \emph{afforded} rather than
forced --- structure the same primitive makes natural and provable
...
exactly the lower polar of that context
...
\code{CompleteLattice} instance). This is where the title's term
lives, and it names the closure, not the primitive: a
\emph{hyperframe} is what a family of set-valued frames generates at
corpus level --- a Galois-closed pair, not any single source's
$S\,i$. Every partial feasibility set is a
...
naive worry about oscillation: if the feasible set somehow
\emph{grows}, the enforced constraint set cannot in fact have grown ---
some constraint must have been \emph{dropped}
...
right object to hand back is not silence but the lattice of maximal
jointly-feasible sub-readings. The exact ancestor of that repair is
Rescher and Manor's reasoning from maximal consistent subsets of an
inconsistent premiss set \citep{rescher1970}; van Fraassen's
supervaluationism supplies the complementary, gap-tolerant half ---
evaluation over all admissible resolutions of what remains
underdetermined \citep{vanfraassen1966supervaluations}. Either way,
disagreement shows up as multiplicity in the lattice rather than as
global collapse.

This is also where the coherence stance parts company with the
classical theory of \emph{semantic information}. Bar-Hillel and
Carnap's measure is truth-anchored --- content is measured against
state-descriptions of the world --- and famously pays for it with the
paradox that a contradiction carries \emph{maximal} information
\citep{barhillel1952semantic}. Floridi's veridicality thesis pays at
the other counter: false content is not semantic information at all
\citep{floridi2011semantic}. A contradictory corpus, for STE, is
neither maximally informative nor informationally void; it has
structured, local, auditable content --- the lattice of sub-readings
just described and, in the local-to-global analysis below, a finite
count of exactly which compatible families fail to glue
(\code{diagonal\_cechObstruction}).
...
of partitions \citep{shannon1953lattice,delsol2024}; the
must-partition of a corpus is the infimum of its feasible partitions
(\code{mustSetoid\_eq\_sInf}), and its rank is monotone in the corpus
(\code{rank\_mustSetoid\_mono}), so more
...
counting shadow of Shannon's information distance
\citep{shannon1953lattice,delsol2024}: symmetric,
...
(\code{couplingExcess\_pos\_of\_not\_le}; the converse is
\code{couplingExcess\_eq\_zero\_iff}).

\paragraph{6. Coherence checking is local-to-global, with a computable
obstruction.}
The last consequence is not geometry but a gluing principle: when does
checking coherence locally certify it globally? Where the corpus really
