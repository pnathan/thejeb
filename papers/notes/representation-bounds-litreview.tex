...
fooling-set bound of K\&N, Ch.~1 --- every monochromatic rectangle
contains at most one fooling point --- is exactly the statement mechanized as
...
the VLSI/communication setting, and itself proved that cover number and
deterministic complexity \emph{are} polynomially related:
$D(f)=O(N^0(f)\cdot N^1(f))$, i.e.\ deterministic complexity is bounded
by the product of the nondeterministic cover numbers for $f$ and its
complement (one of the classical results underlying $\mathrm{P}^{cc}
=\mathrm{NP}^{cc}\cap\mathrm{coNP}^{cc}$). Historically this is where ``representation size
...
method: for almost all Boolean functions the rank method certifies a
linear lower bound while the largest fooling set is only logarithmic in
size, so the fooling-set bound can be exponentially smaller than
deterministic complexity bounds obtained by rank. (In the other
direction the gap is only a constant factor: fooling-set bounds beat
rank-based ones by at most a factor of two.) Relevant
