...
\lean{diagonal\_mixed\_cocycle\_not\_in\_globalRange},
...
gluing implies $R$-linear gluing
(\lean{GluesCover.linearlyGlues}) and rectangular
constraints glue linearly over every cover
(\lean{rectangular\_linearlyGlues}), so the class is a
...
In the mechanization the cokernel statement is rendered at
representative level---``class $=0$ iff the representative cocycle
lies in the image submodule''
(\lean{linearlyGlues\_iff\_mem\_range})---because the literal quotient
type over the submodule coercion $\ker d^1$ hits a typeclass-instance
diamond in current Mathlib (\lean{HasQuotient} fails to synthesize for
these concrete \lean{Finsupp}/\lean{Pi} coefficient modules, while
each component instance synthesizes); the submodule-level formulation
